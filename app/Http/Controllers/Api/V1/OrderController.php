...
class OrderController extends Controller
{
    ...
    public function order_validation(Request $request)
    {
        ...
        if ($intent->status == 'succeeded') {
            ...
            $this->payment::where(['attribute_id' => $request['order_id'], 'attribute' => 'order'])->update([
                'payment_method' => 'stripe',
                'is_paid' => 1,
                'transaction_id' => $intent->id,
            ]);

            $data = $this->payment::where(['attribute_id' => $request['order_id'], 'attribute' => 'order'])->first();
        ...
        $payment_data = $this->payment::where(['attribute_id' => $request['payment_id']])->first();
        if (isset($payment_data) && function_exists($payment_data->failure_hook)) {
            call_user_func($payment_data->failure_hook, $payment_data);
        }
        
        return response()->json(['data' => $payment_data, 'message' => 'fail']);
    }
}
