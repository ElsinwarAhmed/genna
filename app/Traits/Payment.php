...
trait Payment
{
    ...
    public static function generate_payment(object $payer, object $payment_info, Object $receiver)
    {
        if ($payment_info->getPaymentAmount() === 0) {
            throw new InvalidArgumentException('Payment amount can not be 0');
        }
    ...
    public static function processPayment($payment_amount)
    {
        try {
            $config = DB::table('addon_settings')->where('key_name', 'stripe')
        ...
        Stripe::setApiKey($config_values->api_key);
        ...
        try {
            ...
            $paymentIntent = \Stripe\PaymentIntent::create([
                'amount' => round($payment_amount, 2) * 100,
                'currency' => $currency,
                // 'confirmation_method' => 'manual',
                // 'confirm' => true,
            ]);
        } catch(\Stripe\Exception\CardException $e) {
            return ['message' => "A payment error occurred: {$e->getError()->message}"];
        } catch (\Stripe\Exception\InvalidRequestException $e) {
            return ['message' => "An invalid request occurred."];
        } catch (Exception $e) {
            return ['message' => "Another problem occurred, maybe unrelated to Stripe."];
        }
